  ...
  return (
    <div className="fixed inset-0 z-[110] flex items-center justify-center bg-slate-900/80 backdrop-blur-md p-4 animate-fade-in font-sans">
      <div className="bg-white dark:bg-[#0b0f1a] w-full max-w-sm rounded-[2.5rem] shadow-2xl overflow-hidden relative flex flex-col items-center p-10 border border-slate-100 dark:border-slate-800">
        {/* 상단 아이콘 */}
        <div className="w-20 h-20 bg-rose-50 dark:bg-rose-900/30 rounded-3xl flex items-center justify-center shadow-xl mb-8 transition-colors">
          <FiHeart className="text-rose-400 dark:text-rose-500 text-4xl animate-pulse" />
        </div>
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-[13px] font-bold text-slate-400 dark:text-slate-300 leading-relaxed whitespace-pre-wrap">
            {isSender ? (
              <>
                <span className="text-rose-500 dark:text-rose-400 font-black text-base">
                  {receiverPetName}
                </span>{" "}
                님의 응답을 기다리고 있습니다!
              </>
        ...
        {!isSender && (
          <div className="flex w-full gap-4">
            ...
            <button
              ...
              disabled={processing}
              className="flex-1 py-4 bg-rose-500 dark:bg-rose-500 text-white font-black text-[11px] rounded-2xl shadow-xl shadow-rose-500/20 hover:scale-[1.02] active:scale-95 transition-all uppercase tracking-widest italic disabled:opacity-50 flex items-center justify-center gap-2"
            >
        ...
        {processing && !isSender && (
          <div className="absolute inset-0 bg-white/60 dark:bg-[#0b0f1a]/80 backdrop-blur-sm flex items-center justify-center z-50">
            <div className="w-10 h-10 border-[3px] border-slate-100 dark:border-slate-800 border-t-rose-400 rounded-full animate-spin"></div>
          </div>
        )}
      </div>
    </div>
