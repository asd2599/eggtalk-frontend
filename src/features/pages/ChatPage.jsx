...
const ChatPage = () => {
  ...
  const handleSendMessage = async (e) => {
    ...
    setMessages((prev) => [
      ...prev,
      { sender: 'user', text: userMessage, isNew: false },
    ]);
    setInputValue('');
    setIsTyping(true);
    try {
      ...
      <main className="flex-1 flex flex-col lg:flex-row items-stretch p-3 lg:p-8 gap-4 lg:gap-8 bg-slate-50 dark:bg-[#0b0f1a] h-full overflow-y-auto lg:overflow-hidden custom-scrollbar pb-24 lg:pb-8 transition-all relative z-10">
        <div className="w-full lg:w-[380px] flex flex-col flex-shrink-0">
          <div className="flex flex-col items-center">
            <div className="w-32 h-32 lg:w-40 lg:h-40 flex-shrink-0 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-[2rem] lg:rounded-[2.5rem] shadow-sm flex items-center justify-center mb-6 relative overflow-hidden group">
              {petData &&
                petData.draw(
                  'w-20 h-20 lg:w-28 lg:h-28 object-contain relative z-10 transition-transform duration-500 group-hover:scale-110 drop-shadow-xl',
                )}
            </div>

            <div className="text-center mb-4">
              <h2 className="text-lg lg:text-2xl font-black text-slate-900 dark:text-white tracking-tighter italic leading-none uppercase">
                {petData?.name}
              </h2>
              <p className="text-[9px] lg:text-[10px] text-sky-400 font-black mt-3 uppercase tracking-widest leading-none">
                Lv. {petData?.level}
              </p>
            </div>
              ...
              {[
                ...
                <div key={sIdx} className="space-y-3">
                  <div className="border-b border-slate-100 dark:border-slate-800 pb-1 px-1 flex items-center gap-2">
                    ...
                    <h4 className="text-[9px] font-black text-slate-300 dark:text-slate-400 uppercase tracking-[0.3em] italic">
                      {section.title}
                    </h4>
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    {section.stats.map((key) => {
                      ...
                      return (
                        <div
                          key={key}
                          className="flex items-center gap-2.5 p-3 rounded-2xl bg-white dark:bg-slate-900/80 border border-slate-100 dark:border-slate-800 transition-all hover:border-sky-100 dark:hover:border-sky-700 group shadow-sm"
                        >
                          <div className="w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0 bg-slate-50 dark:bg-slate-800 transition-colors group-hover:bg-sky-50 dark:group-hover:bg-sky-900/50">
                            <config.icon
                              className={`text-[12px] lg:text-[14px] ${config.color} group-hover:text-sky-400`}
                            />
                          </div>
                          <div className="flex-1 flex flex-col min-w-0 text-left">
                            ...
                            <span className="text-[11px] font-black text-slate-900 dark:text-slate-100 font-mono tracking-tighter">
                              {petData?.[key]}
                            </span>
                          </div>
                        </div>
        ...
        <div className="flex-1 flex flex-col bg-white dark:bg-[#0b0f1a] border border-slate-100 dark:border-slate-800 rounded-[2.5rem] lg:rounded-[3rem] shadow-sm overflow-hidden relative min-h-[550px] lg:h-full transition-colors">
          <div className="px-6 lg:px-8 py-5 border-b border-slate-50 dark:border-slate-900 bg-white dark:bg-[#0b0f1a] flex justify-between items-center">
            <span className="text-[9px] font-black text-slate-300 dark:text-slate-600 uppercase tracking-[0.4em]">
          ...
          >
            {messages.map((msg, i) => (
              ...
              >
                <div
                  className={`max-w-[90%] lg:max-w-[80%] flex items-start gap-3 lg:gap-4 ${msg.sender === 'user' ? 'flex-row-reverse' : 'flex-row'}`}
                >
                  <div
                    className={`w-9 h-9 lg:w-10 lg:h-10 flex items-center justify-center flex-shrink-0 border overflow-hidden transition-all duration-300 shadow-md ${msg.sender === 'user' ? 'rounded-xl bg-slate-900 border-slate-800 dark:bg-sky-400 dark:border-sky-300' : 'rounded-full bg-white dark:bg-slate-800 border-slate-100 dark:border-slate-700'}`}
                  >
                    {msg.sender === 'user' ? (
                      <FiUser className="text-white dark:text-slate-900 text-sm" />
                    ) : (
                      petData?.draw(
                        'w-full h-full object-contain scale-[1.1] translate-y-0',
                      ) || <FiSmile className="text-sky-400 text-sm" />
                    )}
                  </div>
                  <div className="flex flex-col gap-1.5 min-w-0">
                    <div
                      className={`px-4 py-2.5 lg:px-5 lg:py-3 rounded-[1.2rem] lg:rounded-[1.4rem] text-[13px] lg:text-[14px] font-medium leading-snug shadow-sm transition-all duration-300
                      ${
                        msg.sender === 'user'
                      ...
                      <div className="flex flex-wrap gap-1.5 lg:gap-2 px-1">
                        {Object.entries(msg.analysis).map(([key, val]) => {
                          ...
                          return (
                            <span
                              key={key}
                              className={`text-[9px] font-black px-2.5 py-0.5 rounded-full border bg-white dark:bg-slate-800 border-slate-100 dark:border-slate-700 ${val > 0 ? 'text-sky-400' : 'text-slate-400 dark:text-slate-300'}`}
                            >
          ...
          <div className="p-4 lg:p-8 bg-white dark:bg-[#0b0f1a] border-t border-slate-50 dark:border-slate-900">
            ...
            >
              ...
              <button
                ...
                className="absolute right-3 p-3 bg-slate-900 dark:bg-sky-400 text-white dark:text-slate-950 rounded-2xl hover:scale-105 active:scale-95 transition-all shadow-lg"
              >
