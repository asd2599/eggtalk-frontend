        ...
        <div className="w-full lg:w-[380px] flex flex-col flex-shrink-0">
          <div className="flex flex-col items-center">
            ...
            <div className="text-center mb-4">
              ...
              <p className="text-[9px] lg:text-[10px] text-amber-500 font-black mt-3 uppercase tracking-widest italic leading-none">Lv. {petData?.level}</p>
            </div>

            <div className="w-full space-y-4">
              {[
                { title: "Survival Stats", stats: ["healthHp", "hunger", "cleanliness", "stress"] },
                ...
                <div key={sIdx} className="space-y-2.5">
                  <div className="border-b border-gray-100 dark:border-gray-800 pb-1 px-1">
                    <h4 className="text-[9px] font-black text-gray-300 dark:text-gray-700 uppercase tracking-[0.3em] italic">{section.title}</h4>
                  </div>
                  <div className="grid grid-cols-2 gap-2 lg:gap-3">
                    {section.stats.map((key) => {
                      const config = statConfig[key];
                      return (
                        <div key={key} className="flex items-center gap-2 p-2.5 rounded-2xl bg-white dark:bg-[#0b0f1a] border border-gray-100/50 dark:border-gray-800 shadow-sm transition-all hover:border-gray-400 dark:hover:border-gray-600 group">
                          <div className={`w-7 h-7 rounded-xl flex items-center justify-center flex-shrink-0 ${isDarkMode ? 'bg-[#0b0f1a]' : 'bg-gray-50'}`}>
                            <config.icon className={`text-[12px] lg:text-[14px] ${config.color}`} />
                          </div>
                          <div className="flex-1 flex flex-col min-w-0">
                            <span className="text-[10px] lg:text-[12px] font-bold text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white transition-colors truncate">{config.label}</span>
                            <span className="text-[11px] lg:text-[13px] font-black text-gray-900 dark:text-white font-mono tracking-tighter">{petData?.[key]}</span>
                          </div>
                        </div>
                      );
                    })}
          ...
          <button onClick={handleAnalyzeTendency} disabled={analysisLoading} className="w-full py-4 mt-6 bg-gray-900 dark:bg-white text-white dark:text-gray-900 rounded-2xl font-bold text-[10px] uppercase tracking-[0.2em] active:scale-95 transition-all shadow-lg flex items-center justify-center gap-2">
        ...
        <div className="flex-1 flex flex-col bg-white dark:bg-[#0b0f1a] border border-gray-100 dark:border-gray-800 rounded-[2.5rem] lg:rounded-[3rem] shadow-sm overflow-hidden relative min-h-[550px] lg:h-full mt-4 lg:mt-0 transition-colors">
          ...
          <div className="flex-1 overflow-y-auto p-4 lg:p-8 space-y-4 lg:space-y-6 custom-scrollbar scroll-smooth">
            {messages.map((msg, i) => (
              <div key={i} className={`flex ${msg.sender === "user" ? "justify-end" : "justify-start"} animate-fade-in-up`}>
                <div className={`max-w-[90%] lg:max-w-[85%] flex items-start gap-3 ${msg.sender === "user" ? "flex-row-reverse" : "flex-row"}`}>
                  <div className={`w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0 border ${msg.sender === "user" ? "bg-gray-900 border-gray-800 shadow-md" : "bg-white dark:bg-[#0b0f1a] border-gray-100 dark:border-gray-800 shadow-sm"}`}>
                    {msg.sender === "user" ? <FiUser className="text-white text-xs" /> : <FiSmile className="text-amber-500 text-xs" />}
                  </div>
                  <div className="flex flex-col gap-2 min-w-0">
                    <div className={`p-4 rounded-[1.4rem] text-[12px] lg:text-[13px] leading-relaxed shadow-sm transition-all duration-300
                      ${msg.sender === "user" 
                    ...
                    {msg.sender === "pet" && msg.analysis && (
                      <div className="flex flex-wrap gap-1 lg:gap-2 px-1">
                        {Object.entries(msg.analysis).map(([key, val]) => {
                          ...
                          return (
                            <span key={key} className={`text-[8px] lg:text-[10px] font-black px-2 py-0.5 rounded-full border ${val > 0 ? "bg-emerald-50/50 dark:bg-emerald-900/20 border-emerald-100 dark:border-emerald-800 text-emerald-600 dark:text-emerald-400" : "bg-rose-50/50 dark:bg-rose-900/20 border-rose-100 dark:border-rose-800 text-rose-600 dark:text-rose-400"}`}>
            ...
            {isTyping && <div className="text-[10px] text-amber-500/50 ml-12 italic animate-pulse font-bold tracking-widest">생각 중...</div>}
            <div ref={chatEndRef} />
          </div>
          ...
          <div className="p-4 lg:p-6 bg-white dark:bg-[#0b0f1a] border-t border-gray-50 dark:border-gray-800">
            <form onSubmit={handleSendMessage} className="relative flex items-center max-w-2xl mx-auto w-full group">
              <input type="text" value={inputValue} onChange={(e) => setInputValue(e.target.value)} disabled={isTyping} placeholder="대화를 입력하세요..." className="w-full bg-gray-50 dark:bg-[#0b0f1a] border border-gray-100 dark:border-gray-700 text-gray-900 dark:text-gray-100 text-[12px] px-6 py-4 lg:py-4.5 rounded-2xl focus:outline-none focus:ring-1 focus:ring-gray-400 transition-all shadow-inner" />
              <button type="submit" disabled={isTyping || !inputValue.trim()} className="absolute right-2 p-2.5 bg-gray-900 dark:bg-white text-white dark:text-gray-900 rounded-xl hover:scale-105 active:scale-95 transition-all shadow-lg"><FiSend className="text-xs" /></button>
            </form>
          </div>
        </div>
      </main>
