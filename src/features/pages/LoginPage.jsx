...
const LoginPage = () => {
  ...
  return (
    <>
      <LoadingModal isVisible={isLoading} message="에그톡의 세계로 들어가는 중..." />
        ...
        <div className="w-full max-w-[340px] px-6 relative z-10">
          ...
          <form onSubmit={handleLogin} className="flex flex-col gap-4">
            ...
            <button
              type="submit"
              disabled={isLoading}
              className={`w-full mt-4 bg-slate-900 dark:bg-sky-400 text-white dark:text-slate-950 text-[12px] font-black py-4 rounded-2xl hover:scale-[1.02] active:scale-[0.98] shadow-2xl shadow-sky-500/10 dark:shadow-sky-400/20 uppercase tracking-[0.2em] italic transition-all ${isLoading ? 'opacity-70 cursor-not-allowed transform-none' : ''}`}
            >
              {isLoading ? "로딩 중..." : "시작하기"}
            </button>
          </form>
