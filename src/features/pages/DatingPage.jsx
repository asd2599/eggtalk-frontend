    ...
    <div className="flex flex-col h-screen bg-white dark:bg-[#0b0f1a] transition-colors duration-500 font-sans">
      <header className="h-16 lg:h-20 border-b border-slate-50 dark:border-slate-900 bg-white/90 dark:bg-[#0b0f1a]/90 backdrop-blur-md flex items-center justify-between px-4 lg:px-8 z-50 flex-shrink-0">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-lg lg:text-xl font-black text-slate-900 dark:text-white flex items-center gap-2 italic uppercase tracking-tighter">
              {currentRoomName}
              {!isWaiting && (
                <span className="w-2 h-2 rounded-full bg-sky-400 animate-pulse shadow-[0_0_8px_rgba(125,211,252,0.6)]" />
              )}
            </h1>
            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-0.5">
              {isWaiting ? "연결 대기 중..." : "대화 상대와 연결되었습니다."} (
              {roomUsers.length}/2)
            </p>
          </div>
        </div>
        ...
        {!isWaiting && otherPetInstance && petData && (
          <div className="hidden lg:flex absolute top-5 left-1/2 -translate-x-1/2 items-start gap-4 z-[60]">
            <div
              className={`w-52 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md rounded-[1.8rem] border border-slate-100 dark:border-slate-800 shadow-2xl p-4 transition-all duration-500 mt-2 ${showMyStats ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-4 pointer-events-none"}`}
            >
              <div className="flex items-center justify-between mb-3 border-b border-slate-50 dark:border-slate-800 pb-2">
                ...
                <span className="text-[10px] font-black text-sky-400 bg-sky-50 dark:bg-sky-900/20 px-2.5 py-0.5 rounded-full uppercase">
                  LV.{petData.level}
                </span>
              </div>
            ...
            <div className="flex flex-col items-center">
              <div className="flex items-center gap-4">
                <button
                  ...
                  className={`relative w-14 h-14 rounded-2xl border transition-all hover:scale-110 active:scale-95 shadow-lg flex items-center justify-center p-1 ${showMyStats ? "bg-slate-900 border-slate-800 dark:bg-slate-100 dark:border-white" : "bg-white border-slate-100 dark:bg-slate-900 dark:border-slate-800"}`}
                >
                  <div className="w-full h-full overflow-hidden rounded-xl relative pointer-events-none">
                ...
                <div className="w-9 h-9 rounded-full bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700 flex items-center justify-center shadow-inner z-10 animate-bounce-slight mt-2 pointer-events-none">
                  <span className="text-[10px] font-black text-sky-400 italic">
                    VS
                  </span>
                </div>
                <button
                  onClick={() => setShowOtherStats(!showOtherStats)}
                  className={`relative w-14 h-14 rounded-2xl border transition-all hover:scale-110 active:scale-95 shadow-lg flex items-center justify-center p-1 ${showOtherStats ? "bg-slate-900 border-slate-800 dark:bg-slate-100 dark:border-white" : "bg-white border-slate-100 dark:bg-slate-900 dark:border-slate-800"}`}
                >
                  <div className="w-full h-full overflow-hidden rounded-xl relative pointer-events-none">
              ...
              <span className="text-[9px] font-black text-slate-300 dark:text-slate-600 mt-3 uppercase tracking-[0.2em] italic animate-pulse">
                아바타를 눌러 스탯을 확인해 보세요!
              </span>
            </div>

            <div
              className={`w-52 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md rounded-[1.8rem] border border-slate-100 dark:border-slate-800 shadow-2xl p-4 transition-all duration-500 mt-2 ${showOtherStats ? "opacity-100 translate-x-0" : "opacity-0 translate-x-4 pointer-events-none"}`}
            >
              <div className="flex items-center justify-between mb-3 border-b border-slate-50 dark:border-slate-800 pb-2">
                <span className="text-[10px] font-black text-sky-400 bg-sky-50 dark:bg-sky-900/20 px-2.5 py-0.5 rounded-full uppercase">
                  LV.{otherPetInstance.level}
                </span>
                <span className="text-[10px] font-black text-slate-300 uppercase tracking-widest italic truncate ml-2">
                  {otherPetName}
                </span>
              </div>
              <div className="grid grid-cols-2 gap-x-4 gap-y-2.5">
        ...
        <div className="flex items-center gap-3 lg:gap-4">
          ...
          {!isWaiting && (
            <div className="hidden lg:flex items-center gap-3 bg-slate-50/50 dark:bg-slate-900/50 px-4 py-2 rounded-xl border border-slate-100 dark:border-slate-800 shadow-inner">
              <div
                className={`w-2 h-2 rounded-full ${isAutoCommentEnabled ? "bg-sky-400 shadow-[0_0_8px_rgba(125,211,252,0.6)]" : "bg-slate-300 dark:bg-slate-700"}`}
              />
              <div className="flex flex-col">
                ...
                <div className="flex items-center gap-2 mt-0.5">
                  <span
                    className={`text-[9px] font-black ${isAutoCommentEnabled ? "text-slate-900 dark:text-white" : "text-slate-400"}`}
                  >
                  ...
                  <button
                    ...
                    className={`relative w-8 h-4.5 rounded-full transition-all ${isAutoCommentEnabled ? "bg-sky-400" : "bg-slate-200 dark:bg-slate-700"}`}
                  >
                    <div
                      className={`absolute top-0.5 left-0.5 w-3.5 h-3.5 bg-white rounded-full transition-transform ${isAutoCommentEnabled ? "translate-x-3.5" : "translate-x-0"}`}
                    />
                  </button>
                </div>
              </div>
          ...
          <button
            onClick={toggleTheme}
            className="p-2.5 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800 text-slate-400 hover:text-sky-400 transition-all"
          >
            {isDarkMode ? (
      ...
      <main className="flex-1 overflow-hidden relative flex flex-col items-center bg-slate-50 dark:bg-[#0b0f1a] py-4 lg:py-8 px-2 lg:px-4">
        <div className="w-full max-w-5xl h-full flex flex-col bg-white dark:bg-slate-900/40 border border-slate-100 dark:border-slate-800 rounded-[2.5rem] lg:rounded-[3.5rem] shadow-sm overflow-hidden relative">
          <div
            ref={chatEndRef}
            className="flex-1 overflow-y-auto p-5 lg:p-10 space-y-6 lg:space-y-8 scroll-smooth custom-scrollbar"
          >
            {messages.length === 0 && (
              <div className="h-full flex flex-col items-center justify-center text-slate-300 dark:text-slate-700 opacity-50">
                ...
                <p className="font-black uppercase tracking-[0.3em] italic text-xs">
                  대화를 시작해 보세요!
                </p>
              </div>
            )}
            ...
            {messages.map((msg, idx) => {
              ...
              if (msg.isSystem) {
                return (
                  <div
                    key={idx}
                    className="flex justify-center my-6 animate-fade-in-up"
                  >
              ...
              return (
                ...
                >
                  <div
                    className={`max-w-[85%] lg:max-w-[75%] flex items-start gap-3 lg:gap-4 ${isFromMe ? "flex-row-reverse" : "flex-row"}`}
                  >
                    <div
                      className={`w-9 h-9 lg:w-10 lg:h-10 rounded-xl flex items-center justify-center flex-shrink-0 overflow-hidden shadow-md border transition-transform hover:scale-110 ${isFromMe ? "bg-slate-900 border-slate-800 dark:bg-slate-100 dark:border-white" : "bg-white border-slate-100 dark:bg-slate-800 dark:border-slate-700"}`}
                    >
                      {isPetReply ? (
                        isFromMe ? (
                        ...
                        <div
                          className={`${isFromMe ? "text-sky-300" : "text-slate-400"} w-full h-full flex items-center justify-center`}
                        >
                    ...
                    <div
                      className={`flex flex-col gap-1.5 ${isFromMe ? "items-end" : "items-start"}`}
                    >
                      ...
                      {/* 말풍선 사이즈 */}
                      <div
                        className={`px-4 py-2.5 lg:px-5 lg:py-3 rounded-[1.2rem] lg:rounded-[1.4rem] text-[13px] lg:text-[14px] leading-snug shadow-sm break-words whitespace-pre-wrap transition-all ${
                          isFromMe
                            ? isPetReply
            ...
            {isWaiting ? (
              ...
              >
                ...
                <button
                  ...
                  className="absolute right-2.5 p-3.5 bg-slate-900 dark:bg-slate-100 text-white dark:text-slate-900 rounded-2xl hover:scale-105 active:scale-95 transition-all shadow-xl disabled:opacity-30"
                >
