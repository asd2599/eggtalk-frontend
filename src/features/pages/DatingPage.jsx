  ...
  const handleGiftSuccess = (giftName, targetName, userMessage, aiReply, stats) => {
    ...
    if (stats && typeof stats === "object") {
      statLog = Object.entries(stats)
        ...
        .map(([key, val]) => `${statNameMap[key] || key}(${val > 0 ? "+" : ""}${val})`)
        .join(", ");
    }
    ...
    setMessages((prev) => [
      ...prev,
      { sender: "시스템", message: systemNotice, timestamp: new Date(), isSystem: true },
      aiMessageObj,
    ]);
      ...
      <header className="h-16 lg:h-20 border-b border-slate-50 dark:border-slate-900 bg-white/90 dark:bg-[#0b0f1a]/90 backdrop-blur-md flex items-center justify-between px-4 lg:px-8 z-50 flex-shrink-0">
        ...
        <div className="flex items-center gap-3 lg:gap-4">
          <button onClick={handleSendFriendRequest} disabled={isWaiting || !otherPetName || isSendingRequest} className="flex items-center gap-2 px-4 py-2.5 bg-slate-50 dark:bg-slate-900 text-slate-500 dark:text-slate-400 rounded-xl text-[11px] font-black border border-slate-100 dark:border-slate-800 transition-all hover:bg-slate-900 hover:text-white dark:hover:text-slate-100 dark:hover:text-slate-900 disabled:opacity-30 disabled:pointer-events-none uppercase tracking-widest">
            <FiUserPlus className="text-sm" /> <span>친구</span>
          </button>
          <button onClick={() => setIsGiftModalOpen(true)} disabled={isWaiting || !otherPetName} className="flex items-center gap-2 px-4 py-2.5 bg-slate-900 dark:bg-slate-100 text-white dark:text-slate-900 rounded-xl text-[11px] font-black shadow-xl transition-all hover:scale-105 active:scale-95 disabled:opacity-30 disabled:pointer-events-none uppercase tracking-[0.2em] italic">
            <FiGift className="text-sm" /> <span>교감</span>
          </button>
              ...
              if (msg.isSystem) {
                return (
                  <div key={idx} className="flex justify-center my-6 animate-fade-in-up">
                    <span className="px-5 py-2 bg-slate-50 dark:bg-slate-800 text-slate-400 dark:text-slate-500 text-[10px] lg:text-[11px] rounded-full font-black uppercase tracking-widest italic border border-slate-100 dark:border-slate-700">{msg.message}</span>
                  </div>
                );
              }
