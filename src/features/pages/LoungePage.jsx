...
const LoungePage = () => {
  ...
  return (
    <div className="flex flex-col lg:flex-row h-screen bg-white dark:bg-[#0b0f1a] transition-colors duration-500 font-sans overflow-hidden">
      ...
      >
        {isDarkMode ? (
          <FiSun className="text-sm text-amber-500" />
        ) : (
          <FiMoon className="text-sm text-indigo-500" />
        )}
      </button>
        ...
        <div className="flex flex-col lg:flex-row justify-between items-center gap-4 bg-white dark:bg-[#0b0f1a] border border-gray-100 dark:border-gray-800 p-6 lg:px-10 lg:py-7 rounded-t-[2.5rem] lg:rounded-t-[3rem] shadow-[0_-1px_0_rgba(0,0,0,0.05)] border-b-0 flex-shrink-0 transition-colors">
          <div className="text-center lg:text-left">
            <div className="flex items-center justify-center lg:justify-start gap-3">
              <h1 className="text-xl lg:text-2xl font-black text-gray-900 dark:text-white tracking-tighter italic">
                Lounge Area
              </h1>
              <span className="flex h-2 w-2 rounded-full bg-amber-500 animate-pulse"></span>
            </div>
            <p className="text-sm lg:text-[11px] font-bold text-gray-400 mt-1 uppercase tracking-[0.2em]">
              다른 펫들과 실시간으로 1:1 대화를 나눠보세요.
            </p>
          </div>
          ...
          <div className="flex items-center gap-3 w-auto lg:w-auto">
            ...
            <button
              onClick={fetchRooms}
              className="p-3 bg-gray-50 dark:bg-gray-900 border border-gray-100 dark:border-gray-800 text-gray-500 hover:text-amber-500 rounded-2xl transition-all shadow-sm active:scale-95"
            >
            ...
            <button
              ...
              className="p-3 bg-gray-900 dark:bg-white text-white dark:text-gray-900 rounded-2xl font-black hover:scale-105 active:scale-95 transition-all shadow-xl flex items-center justify-center"
            >
        ...
        <div className="flex-1 bg-white dark:bg-[#0b0f1a] border border-gray-100 dark:border-gray-800 rounded-b-[2.5rem] lg:rounded-b-[3rem] shadow-sm p-6 lg:p-10 overflow-y-auto custom-scrollbar transition-colors">
          {Object.keys(rooms).length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-gray-300 space-y-4 py-20">
              ...
              <p className="text-[12px] font-bold tracking-tight uppercase opacity-50">
                만들어진 방이 없습니다.
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
              {Object.entries(rooms).map(([roomId, room]) => {
                ...
                return (
                  <div
                    key={roomId}
                    className="flex flex-col justify-between p-7 bg-white dark:bg-[#0b0f1a] border border-gray-100 dark:border-gray-800 rounded-[2rem] transition-all hover:border-amber-400 dark:hover:border-amber-500 group hover:shadow-2xl hover:shadow-amber-100/10"
                  >
                    <div>
                      <div className="flex justify-between items-start mb-6">
                        <h3 className="text-lg font-black text-gray-900 dark:text-white break-words pr-2 leading-tight">
                          {room.name}
                        </h3>
                        <span
                          className={`px-2.5 py-1 text-[9px] font-black rounded-full uppercase ${isFull ? "bg-rose-50 text-rose-500 border border-rose-100" : "bg-emerald-50 text-emerald-500 border border-emerald-100"}`}
                        >
                      ...
                      <div className="space-y-2">
                        {room.users.map((u, i) => (
                          <div
                            key={i}
                            className="text-[11px] font-bold text-gray-400 flex items-center gap-2 bg-slate-50/50 dark:bg-gray-900/50 py-1.5 px-3 rounded-lg border border-slate-100 dark:border-gray-800"
                          >
                            <FiSmile className="text-amber-500" />{" "}
      ...
      {showCreateModal && (
        <div className="fixed inset-0 z-[100] bg-[#0b0f1a]/60 backdrop-blur-xl flex items-center justify-center p-6 animate-fade-in">
          <div className="bg-white dark:bg-[#0b0f1a] w-full max-w-md rounded-[3rem] p-10 shadow-2xl border border-gray-100 dark:border-gray-800 relative animate-fade-in-up">
            ...
            <p className="text-[11px] font-bold text-gray-400 mb-8 uppercase tracking-widest italic">
              Open a new communication line
            </p>
            <form onSubmit={handleCreateRoom}>
              ...
              <div className="flex gap-4">
                <button
                  ...
                  className="flex-1 py-4 bg-gray-100 dark:bg-gray-900 text-gray-500 rounded-2xl font-black text-[10px] uppercase tracking-widest"
                >
                  취소
                </button>
                <button
                  ...
                  className="flex-1 py-4 bg-gray-900 dark:bg-white text-white dark:text-gray-900 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:scale-105 transition-all shadow-xl shadow-gray-200 dark:shadow-none"
                >
                  방 만들기
                </button>
              </div>
