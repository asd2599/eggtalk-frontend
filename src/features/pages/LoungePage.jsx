        ...
        <div className="max-w-[1000px] mx-auto">
          ...
          <header className="flex flex-col lg:flex-row items-center justify-between gap-6 mb-12 border-b border-slate-100 dark:border-slate-900 pb-8">
            <div className="text-center lg:text-left">
              ...
              <p className="text-[10px] text-slate-400 dark:text-slate-500 font-bold uppercase tracking-[0.3em] mt-3 italic">Live Communication Hub</p>
            </div>

            <div className="flex items-center gap-3">
              <button onClick={fetchRooms} className="p-3 bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800 text-slate-400 hover:text-sky-400 rounded-2xl transition-all shadow-sm active:scale-95">
                <FiRefreshCw className="text-lg" />
              </button>
              <button onClick={() => setShowCreateModal(true)} className="p-3 bg-slate-900 dark:bg-sky-400 text-white dark:text-slate-950 rounded-2xl font-black hover:scale-105 active:scale-95 transition-all shadow-xl flex items-center justify-center">
                  ...
                  <div key={roomId} className="flex flex-col justify-between p-7 bg-white dark:bg-slate-900/50 border border-slate-100 dark:border-slate-800 rounded-[2.5rem] transition-all hover:border-sky-400 dark:hover:border-sky-500 group shadow-sm hover:shadow-2xl hover:shadow-sky-500/5 dark:hover:shadow-none relative overflow-hidden">
                    ...
                    <div>
                      <div className="flex justify-between items-start mb-6">
                        <h3 className="text-lg font-black text-slate-900 dark:text-slate-100 pr-2 leading-tight uppercase tracking-tighter group-hover:text-sky-500 dark:group-hover:text-sky-400 transition-colors">{room.name}</h3>
                        <span className={`px-2.5 py-1 text-[9px] font-black rounded-full uppercase ${isFull ? "bg-slate-100 dark:bg-slate-800 text-slate-400 dark:text-slate-500" : "bg-sky-50 dark:bg-sky-900/40 text-sky-500 dark:text-sky-400 border border-sky-100/50 dark:border-sky-800/50"}`}>
                          {room.users.length}/2
                        </span>
                      </div>
      ...
      {showCreateModal && (
        <div className="fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-6 animate-fade-in font-sans">
          <div className="bg-white dark:bg-[#0b0f1a] w-full max-w-md rounded-[3rem] p-10 shadow-2xl border border-slate-100 dark:border-slate-800 relative animate-scale-in">
            ...
            <p className="text-[10px] font-bold text-slate-400 dark:text-slate-500 mb-8 uppercase tracking-widest italic">Initialize new communication link</p>
