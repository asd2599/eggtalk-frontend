      ...
      <div className="bg-white dark:bg-[#0b0f1a] rounded-[3rem] lg:rounded-[4rem] p-8 lg:p-14 shadow-2xl lg:shadow-none border border-slate-100 dark:border-slate-900 flex flex-col lg:flex-row gap-12 lg:gap-20 items-stretch h-auto min-h-max mb-1 transition-colors duration-500 overflow-hidden relative z-10">
        ...
        <div className="w-full lg:w-[320px] flex flex-col items-center lg:items-stretch flex-shrink-0">
          <div className="aspect-square w-full max-w-[260px] lg:max-w-none bg-slate-50 dark:bg-slate-900/50 rounded-[3rem] border border-slate-100 dark:border-slate-800 mb-8 flex items-center justify-center shadow-inner relative overflow-hidden group">
            {petData &&
              petData.draw(
                'w-40 h-40 lg:w-52 lg:h-52 group-hover:scale-110 transition-transform duration-500 drop-shadow-2xl',
              )}
          </div>

          <div className="text-center lg:text-left mb-10">
            <h2 className="text-2xl lg:text-3xl font-black text-slate-900 dark:text-white tracking-tighter italic leading-none uppercase">
              {petData?.name}
            </h2>
            <div className="flex items-center justify-center lg:justify-start gap-2 mt-3">
              <span className="text-[10px] lg:text-[11px] text-sky-400 font-black uppercase tracking-[0.3em]">
                Level {petData?.level}
              </span>
          ...
          <div className="w-full space-y-8">
            <div className="space-y-3 px-1">
              ...
              <div className="w-full h-1.5 bg-slate-100 dark:bg-slate-900 rounded-full overflow-hidden shadow-inner border border-slate-200/50 dark:border-slate-800">
                <div
                  className="h-full bg-sky-200 dark:bg-sky-900 shadow-[0_0_10px_rgba(186,230,253,0.3)] transition-all duration-1000"
          ...
          {[
            {
              ...
              stats: [
                { label: '체력', value: petData?.healthHp },
                { label: '배고픔', value: petData?.hunger },
                { label: '청결도', value: petData?.cleanliness },
                { label: '스트레스', value: petData?.stress },
              ],
            },
            {
              title: 'Cognitive Matrix',
              stats: [
                { label: '애정도', value: petData?.affection },
                { label: '지식', value: petData?.knowledge },
                { label: '공감', value: petData?.empathy },
                { label: '논리', value: petData?.logic },
                { label: '이타성', value: petData?.altruism },
              ],
            },
            {
              title: 'Behavioral Profile',
              stats: [
                { label: '외향성', value: petData?.extroversion },
                { label: '개방성', value: petData?.openness },
                { label: '직설성', value: petData?.directness },
                { label: '호기심', value: petData?.curiosity },
                { label: '유머', value: petData?.humor },
              ],
            },
          ].map((section, idx) => (
            <section key={idx}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 lg:gap-x-12 gap-y-6">
                {section.stats.map((stat, sIdx) => (
                  ...
                  >
                    <span className="text-[12px] text-slate-500 dark:text-slate-400 font-bold group-hover:text-slate-900 dark:group-hover:text-sky-100 transition-colors w-14 shrink-0">
                      {stat.label}
                    </span>
