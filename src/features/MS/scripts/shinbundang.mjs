...
/**
 * [SYSTEM PRESET] 지하철 좌표 수집 자동화 스크립트 (shinbundang)
...
async function run() {
  ...
  console.log(`[START] Fetching coordinates for shinbundang (${target_stations.length} stations)`);
  let out = {};
  ...
  if (Object.keys(out).length === 0) {
    console.log(`[DONE] No new stations for shinbundang.`);
    return;
  }
  ...
  text += `\n\n  // --- Automatically Added (shinbundang) ---\n`;
