          ...
          if (res.data?.realtimePositionList) {
            return res.data.realtimePositionList
              .map((item, index) => {
                ...
                const coords =
                  SUBWAY_STATION_COORDS_V2[lineName]?.[rawStation] ||
                  SUBWAY_STATION_COORDS_V2[lineName]?.[rawStation + "역"];
                if (coords) {
                  return {
  ...
  useEffect(() => {
    ...
    const update = () => {
      setPetPosition((prev) => {
        ...
        let nLat = prev.lat,
          nLng = prev.lng;
        if (keys.ArrowUp) nLat += 0.00005;
        if (keys.ArrowDown) nLat -= 0.00005;
        if (keys.ArrowLeft) nLng -= 0.00006;
        if (keys.ArrowRight) nLng += 0.00006;
        return nLat !== prev.lat || nLng !== prev.lng
          ? { lat: nLat, lng: nLng }
          : prev;
      });
      ...
      <main className="flex-1 relative overflow-hidden">
        ...
        <div className="absolute top-6 left-8 md:left-10 z-50 flex flex-row items-start gap-4 pointer-events-none transition-all duration-300">
          ...
          <div className="flex flex-col items-start gap-1.5 pointer-events-auto">
            ...
            >
              <i
                className={`ri-route-line text-lg ${showSearch ? "text-white" : "text-sky-500"}`}
              ></i>
              <div className="flex flex-col items-start pr-1">
                ...
                <span
                  className={`text-[6px] font-bold uppercase tracking-widest leading-none ${showSearch ? "text-blue-200" : "text-slate-400"}`}
                >
                  Route Finder
                </span>
              </div>
        ...
        <div className="absolute top-6 right-8 md:right-10 z-50 pointer-events-none transition-all duration-300">
          <button
            onClick={() => {
              setIsSubwayRealtimeOn(!isSubwayRealtimeOn);
              if (isSubwayRealtimeOn) setSubways([]);
            }}
          ...
          >
            <i
              className={`ri-broadcast-line text-lg ${isSubwayRealtimeOn ? "animate-pulse text-white" : "text-slate-400"}`}
            ></i>
            <div className="flex flex-col items-start">
              ...
              <span
                className={`text-[6px] font-bold uppercase tracking-widest leading-none ${isSubwayRealtimeOn ? "text-blue-200" : "text-slate-400"}`}
              >
                Realtime
              </span>
            </div>
                ...
                >
                  <div
                    onClick={(e) => {
                      e.stopPropagation();
                      setSelectedTrainId(
                        selectedTrainId === sub.id ? null : sub.id,
                      );
                    }}
