...
// //* [Modified Code] 지도는 메르카토르 투영법(정각도법)을 쓰기 때문에, 시각적 각도를 맞추기 위해 위도에 따른 경도 거리 비율(cos) 보정 적용
const calculateBearing = (startLat, startLng, endLat, endLng) => {
  const latRad = startLat * (Math.PI / 180);
  const dy = endLat - startLat;
  const dx = (endLng - startLng) * Math.cos(latRad);
        ...
        try {
          ...
          if (res.data?.realtimePositionList) {
            ...
            return items.map((item, index) => {
              ...
              // //* [Added Code] 명칭 예외 처리 (불암산 -> 당고개, 이수 <-> 총신대입구 등)
              if (!stationCoords) {
                let aliasName = stationName;
                if (stationName === '불암산') aliasName = '당고개';
                if (lineName === '7호선' && stationName === '총신대입구')
                  aliasName = '이수';
                if (lineName === '4호선' && stationName === '이수')
                  aliasName = '총신대입구';
                  ...
                  if (
                    ...
                    if (pathPoints && pathPoints.length > 0) {
                      ...
                      if (sttus === '2') {
                        ...
                        pointIndex = -1; // //* [Modified Code] 현재역 정차 상태에서는 역 좌표(baseLat, baseLng) 유지
                      ...
                      // //* [Modified Code] 화살표 방향(Bearing)도 종착역이 아닌, '눈앞의 가장 가까운 철로 곡선 점'을 바라보게 보정
